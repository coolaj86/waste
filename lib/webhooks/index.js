...
myMailgun.secureWebhooks = function (config) {
  var mailgunTokens = {}
  ...
  return function (req, res, next) {
    if (!testSignature(config.mailer.apiKey, req.body.timestamp, req.body.token, req.body.signature)) {
...
module.exports.create = function (app, config) {
  ...
  app.use(config.webhookPrefix + '/twilio', myTwilio.secureWebhooks(config));
  app.use(config.webhookPrefix + '/twilio', myTwilio.session());

  app.use(config.webhookPrefix + '/mailgun', myMailgun.secureWebhooks());
