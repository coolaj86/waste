...
module.exports.create = function (opts) {
  ...
  function forwardSmsViaEmail(texter, sms, raw) {
    ...
    sender = contact.name || formatNumber(texter);
    ...
    */
    ...
    return mailer.send({
      from: (contact.name || texter) + ' <' + texter + '@' + opts.smsdomain + '>'
    //, to: event.email
    , to: opts.mailer.opts.defaults.replyTo
    , bcc: opts.mailer.opts.defaults.bcc
    , replyTo: texter + '@' + opts.smsdomain
    // replyTo: (contact.name || texter) + ' <' + texter + '@sms.itstitle.com>'
    , subject: subject
    , text: msg
    });
  }
