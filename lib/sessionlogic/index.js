...
module.exports.init = function (app, passport, config, Auth) {
  ...
  function handleLogin(req, res, next, opts) {
    ...
    function afterLogIn(err) {
      ...
      if (opts.callback) {
        // TODO confirm that req.user.login === opts.user
        // twitter needs this callback to determine
        // if this user has been authenticated AND authorized
        opts.callback(opts.user, finish);
      } else {
        finish();
      }
    }

    if (false === opts.session) {
      return Auth.handleNewLogin(session).then(function () {
        req.user = session;
        afterLogIn(null);
      });
    }
  ...
  localRoute = local.create(passport, config, opts).route;
  bearerRoute = bearer.create(passport, config, opts).route;
  ...
  routes.push(bearerRoute);

  // 3rd Party Providers
  routes.push(loopback.init(passport, config, opts));
  ...
  app
    ...
    .use(function (req, res, next) {
        ...
        passport.authenticate('bearer.st', function (err, login, info) {
          if (err) {
            res.error({
  ...
  return {
    manualLogin: localRoute.manualLogin
