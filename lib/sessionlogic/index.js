...
module.exports.init = function (app, passport, config, Auth) {
  function getLoginFromResults(loginResults) {
    if (loginResults.user.$login) {
      return PromiseA.resolve(loginResults.$login);
    }

    if (!loginResults.user.profile) {
      return PromiseA.reject(new Error("user.$login was not set, neither user.profile"));
    }

    // TODO upsert oauthy-thingy?
    return loginHelpers.Auth.Oauth2Login
  ...
  function loginHelpersLogin(req, res, next, loginResults) {
    ...
    getLoginFromResults(loginResults).then(function () {
      // XXX
      // TODO make sure we have $login or $token or profile
      // XXX
      if (!loginResults.user.$login) {
        console.error('[ERROR] loginHelpersLogin');
  ...
  function loginWithBearerToken(req, res, next) {
    ...
    passport.authenticate('bearer.st', function (err, user, info) {
      ...
      loginHelpers.login(req, res, next, loginResults);
    })(req, res, next);
  }

  function controlApiAccess(req, res, next) {
    req.client = req.client || {}; // TODO link all logins to a client
    req.client.config = { stripe: config.stripe }; // TODO - for stripe token and such
    ...
    if (!req.user.$token) {
      if (!config.trustedOrigins.some(function (origin) {
        ...
        return re.test(req.headers.origin);
      })) {
        res.error({
  ...
  var publicApiRe = new RegExp('^' + config.publicApi
    ...
    , loginHelpers = { Auth: Auth, login: loginHelpersLogin }
      // TODO test that publicApi actually falls under apiPrefix
    , localHelpers = local.create(passport, config, loginHelpers)
    , bearerHelpers = bearer.create(passport, config, loginHelpers)
    ;
  ...
  function route(rest) {
    ...
    //rootUser.init(passport, config, loginHelpers)(rest);

    localHelpers.route(rest);
    bearerHelpers.route(rest);

    // Faux 3rd Party Provider (for testing)
    loopback.init(passport, config, loginHelpers).route(rest);

    // 3rd Party Providers
    //facebook.init(passport, config, loginHelpers)(rest);
    //twitter.init(passport, config, loginHelpers)(rest);
    //tumblr.init(passport, config, loginHelpers)(rest);
    //ldsconnect.init(passport, config, loginHelpers)(rest);
  }
