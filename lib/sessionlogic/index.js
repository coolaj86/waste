...
module.exports.init = function (app, passport, config, Auth) {
  ...
  app
    ...
    .use(function (req, res, next) {
        ...
        if (!/^bearer/i.test(req.headers.authorization) && !(/\?access_token=/.test(req.url) || req.query.access_token)) {
          next();
          return;
        }
        ...
        passport.authenticate('bearer.st', function (err, user, info) {
          ...
          if (!user.account) {
            res.send({ error: {
              message: "Valid login, but Invalid account"
            , code: 401
  ...
  function route(rest) {
    ...
    results.routes.forEach(function (fn) {
      // TODO
