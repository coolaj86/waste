...
module.exports.create = function (app, config, Db) {
  ...
  Contacts.restful.get = function (req, res) {
    ...
    if (!uuid) {
      Db.Contacts.forge({ uuid: UUID.v4(), account_uuid: req.user.account.id })
      ...
      Db.ContactNodes.forge().where({ contact_uuid: uuid }).fetchAll()
        .then(function (nodes) {
          res.json({
  ...
  Contacts.restful.delete = function (req, res) {
    Db.ContactNodes.forge().where({ uuid: req.params.uuid })
