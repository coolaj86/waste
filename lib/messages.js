...
module.exports.create = function (Db/*app, config, Auth, manualLogin*/) {
  ...
  function route(rest) {
    ...
    function messageBuilder (account, event) {
    ...
    Messages.send = function () {
      return Messages.find({ whereNull: 'sent' }).then(function (messages) {
        // TODO: run messageBuilder() then actually send via push, email, etc. and use Promise.all
      });
    };
    ...
    rest.get('/messages/me', getMyMessages);
  }
