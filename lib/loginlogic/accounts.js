...
module.exports.create = function (opts) {
  ...
  Accounts.create = function (loginIds, meta, cb) {
    ...
    // TODO better typing
    account = { id: uuid, uuid: uuid, loginIds: loginIds, xattrs: {} };
  ...
  Accounts.unlink = function (session, accountId, loginId, cb) {
    function unlinkLoginFromAccount(account) {
      // remove login from account
      account.loginIds.some(function (_loginId, i) {
        if (loginId === _loginId) {
          account.loginIds.splice(i, 1);
          return true;
        }
      });
    }
    unlinkLoginFromAccount(session.account);
    Accounts.getOne(accountId, function (err, account) {
      unlinkLoginFromAccount(account);
  ...
  Accounts.getOne = function (uuid, cb) {
    Accounts.mget([uuid], function (err, accounts) {
      cb(err, accounts && accounts[0] || null);
    });
  };
