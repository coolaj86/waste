...
module.exports.Users.create = function (opts) {
  ...
  Users.getOne = function (loginId, cb) {
    Users.mget([loginId], function (err, logins) {
      cb(err, logins && logins[0] || null);
    });
  };
...
module.exports.create = function (opts) {
  ...
  UsersWrapper.getOne = Users.getOne;
  ...
  UsersWrapper.unlink = function (session, loginId, accountId, cb) {
    function unlinkAccountFromLogin(login) {
      login.accountIds.some(function (_accountId, j) {
        if (accountId !== _accountId) {
          return false;
        }

        login.accountIds.splice(j, 1);
        return true;
      });
    }
    session.logins.some(function (login) {
      if (loginId === login.id) {
        unlinkAccountFromLogin(login);
        return true;
      }
    });
    UsersWrapper.getOne(loginId, function (err, login) {
      unlinkAccountFromLogin(login);
      Users.save(login);
