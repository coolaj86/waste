...
module.exports.create = function (config, Logins, Accounts) {
  ...
  function serialize(sessionObj, done) {
    ...
    function handleNewLogin() {
      ...
      if (!sessionObj.selectedAccountId) {
        sessionObj.selectedAccountId = login.primaryAccountId;
      }

      Accounts.mget(login.accountIds || [], function (err, accounts) {
  ...
  function unlink(session, accountId, loginId, cb) {
    Accounts.unlink(session, accountId, loginId, function () {
      Logins.unlink(session, loginId, accountId, function () {
        if (cb) { cb(); }
      });
    });
  }
