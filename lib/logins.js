...
module.exports.createController = function (config, DB, ContactNodes) {
  ...
  */
  ...
  Logins.sendEmailCode = function (mailconf, subject, message, $code) {
    ...
    //    + ' <' + texterEmail + '>'
    , subject: subject.replace(/{{\s+code\s+}}/, $code.get('code'))
    , text: message.replace(/{{\s+code\s+}}/, $code.get('code'))
    //, html: ""
    });
  };
  ...
  Logins.sendPhoneCode = function (twilconf, sms, $code) {
    ...
    , body: sms.replace(/{{\s+code\s+}}/, $code.get('code'))
    });
  };
  ...
  */
  ...
  Logins.getAndSendClaimCode = function (conf, type, node) {
    ...
    messages.subject = "EMAIL {{ code }} is your verification code";
  ...
  Logins.check = function (type, node) {
    ...
    type = type || ContactNodes.getNodeType(fnode);
    var id = fnode && secretutils.md5sum(fnode);
...
module.exports.createView = function (config, DB, manualLogin, ContactNodes) {
  ...
  Logins.restful.create = function (req, res) {
    ...
    var promise = Logins.create(loginnodes, secret, { recoverynodes: recoverynodes }).then(function ($login) {
  ...
  Logins.restful.check = function (req, res) {
    ...
    // TODO check type
    var promise = Logins.check(req.params.type, req.params.node).then(function (exists) {
  ...
  Logins.restful.login = function (req, res) {
    ...
    var promise = Logins.login(type, node, secret).then(function ($login) {
      var manualLoginWrapped = wrapManualLogin(req, res);
