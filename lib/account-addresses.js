...
module.exports.createController = function (defaultConfig, DB) {
  ...
  Addresses.remove = function (config, $account, $addresses, addrId) {
    // TODO destroy?
    return $addresses.detach([addrId]);
  };
  ...
  Addresses.update = function (config, $account, $addresses, updates, addrId) {
    ...
    // TODO types should be data-related 'residential|commercial|pobox',
    // not state-related 'shipping|billing|work|home|winter_cabin'
    //delete updates.type;

    //address = new DB.Addresses(req.body.addrId)
    //address.save(req.body, { patch: true }).then(function (address)

    console.log("UPDATE ADDR", addrId);
    console.log(addrId);

    console.log('$addresses');
    console.log($addresses);
    $addresses.some(function ($_address, i) {
      console.log("ADDRESS", i, $_address.id);
...
*/
  ...
  Addresses.upsertShipping = function (config, $account, $addresses, updates, addrId) {
    var $addrs = $account.related('addresses')
    , ps = []
    ;

    updates.type = 'home';
    $addrs.forEach(function ($addr) {
      if ('home' === $addr.get('type') && $addr.id !== (addrId || updates.id)) {
        $addr.set('type', null);

        ps.push($addr.save());
      } else {
        ps.push(PromiseA.resolve());
      }
    });
