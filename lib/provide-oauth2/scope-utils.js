...
// TODO this needs to go into config.js
scopeGroups = {
  'me': {}
, 'friends': {}
, 'groups': {}
...
exports._delta = function (grantedScope, requestedScope) {
  ...
  return { scope: newScope, exists: exists };
};

exports.delta = function (grantedScope, opts, cb) {
  ...
  cb(null, newScope, parsed.invalid, parsed.scopes, newScope.exists);
};
...
function getScopeDelta(grantedScope, reqScopeArr) {
...
/*
exports.get = function (accountId, AppId, cb) {
  cb(null, store.get(accountId + ':' + AppId));
};

exports.set = function (accountId, AppId, val, cb) {
  var existing = store.get(accountId + ':' + AppId)
    ;

  val = exports.merge(existing, val);
  store.set(accountId + ':' + AppId, val);
  if (cb) {
    cb(null);
  }
};
...
module.exports.getScopeDelta = getScopeDelta;
