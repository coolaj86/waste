...
module.exports.create = function (app, passport, config, DB, Auth) {
  ...
  */
  ...
  server.exchange(oauth2orize.exchange.clientCredentials(function (auth, scope, done) {
    ...
    DB.Accesstokens.forge().save(values, { method: 'insert' }).then(function ($token) {
  ...
  function route(rest) {
    ...
    , passport.authenticate(
        ['provider.oauth2-basic.st', 'provider.oauth2-client-password.st']
      , { session: false }
      )
    , server.token()
    , server.errorHandler()
