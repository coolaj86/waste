...
module.exports.create = function (app, passport, config, DB, Auth) {
    ...
    */
    ...
    return DB.Accounts
      ...
      .then(function ($account) {
        ...
        , grantType: grantValues.grantType || 'code'
        }).then(function ($token) {
  ...
  server.exchange(oauth2orize.exchange.password(function ($apikey, username, password, scope, done) {
    // $apikey will exist, otherwise the previous middleware would have handled the failure already
    var apikey = $apikey.toJSON && $apikey.toJSON() || $apikey
    ...
    , oaouthclientUuid: client.id || client.uuid
    //, $oauthclient: $client
    , apikeyId: apikey.id
    //, $apikey: $apikey
    }).then(function ($user) {
      var user = $user && $user.toJSON && $user.toJSON() || user
        , values
        ;
      ...
      values = {
        scope: scope
      , apikeyId: apikey.id
      , oauthclientUuid: client.uuid || client.id
