...
module.exports.create = function (app, passport, config, DB, Auth) {
  ...
  function createAccessToken(values) {
    var token = utils.uid(256)
      , id = authutils.hashsum('sha256', token)
      ;

    values.id = id;
    // TODO, shouldn't need the original token
    values.token = token;

    return DB.Accesstokens.forge().save(values, { method: 'insert' });
  }
  ...
  server.grant(oauth2orize.grant.code(function (consumer, redirectURI, user, ares, done) {
    console.log('[B-21-C] [grant code]', consumer.uid || consumer.get('uid'), user.account.id);
    ...
    */
    ...
    db.permissions.set($account.id, (consumer.uid || consumer.get('uid')), ares.newScope);
  ...
  server.grant(oauth2orize.grant.token(function (consumer, user, ares, done) {
    console.log('[0?a] [grant token]', consumer.uid || consumer.get('uid'));
    ...
    values = {
      appId: consumer.uid || consumer.get('uid')
    , accountId: $account.id || $account.uuid
    , scope: ares.scope
    };
  ...
  server.exchange(oauth2orize.exchange.code(function (consumer, code, redirectURI, done) {
    ...
    console.log('[B-24-C] [exchange code]');
    //, token = utils.uid(256)
  ...
  server.exchange(oauth2orize.exchange.password(function (clientAuth, username, password, scope, done) {
    var client = clientAuth.oauthclient
      ;
    ...
    , clientAuth: clientAuth
      ...
      values = {
        scope: scope
      , apikeyId: clientAuth.id
      , oauthclientUuid: client.uuid
      , loginId: user.id || user.uuid || user.hashid
      , accountUuid: null
      , test: clientAuth.test
      , insecure: clientAuth.insecure
      };

      createAccessToken(values).then(function ($token) {
  ...
  server.exchange(oauth2orize.exchange.clientCredentials(function (auth, scope, done) {
  ...
  function route(rest) {
    ...
    function getScopeDelta(accountId, consumerId, rawScope) {
      console.log('getScopeDelta', accountId, consumerId);
        ...
        , function (err, newScope, invalids, reqScope, exists) {
            ...
            */
            ...
            if (0 === Object.keys(newScope.scope).length) {
              ...
              console.log("Object.keys(newScope.scope)");
              console.log(Object.keys(newScope.scope));
              info.granted = false;
            }
    ...
    , function restfulGetScopeDelta(req, res) {
        ...
        /*
        console.log('[1] accountId, consumer.uid (appId)');
        console.log(data.accountId, data.appId);
        console.log('[1] rawScope', data.rawScope);
        */
    ...
    rest.get(
      // '/dialog/authorize' // TODO make sure nothing references the old place
    ...
    , server.authorization(
        function checkAppIdAndUrl(appId, redirectURI, scope, type, done) {
          // Step 1
          console.log('[A-03-C] [authorize.validateUrl]');
      ...
      , function checkAuthorizationAndScope(consumer, user, rawScope, done) {
          ...
          var $account = user && getUserAccount(user)
          ...
          */
          ...
          /*
          console.log('[0] accountId, consumer.uid');
          console.log(accountId, consumer.uid);
          console.log('[0] rawScope');
          console.log(rawScope);
          */

          //return getScopeDelta(user, consumer, rawScope)
          return getScopeDelta(accountId, consumer.uid, rawScope)
    ...
    // a response.
    decisions = server.decision(function parseDecision(req, done) {
    ...
    rest.post(
      // '/dialog/authorize/decision' // TODO make sure this isn't used
