...
module.exports.create = function (app, passport, config, DB, Auth) {
  ...
  server.grant(oauth2orize.grant.code(function (consumer, redirectURI, user, ares, done) {
    console.log('[grant code]', consumer.id, user.id);
    ...
    console.log('[grant] values', consumer.id, user.id);
    console.log(values);
    ...
    console.log(user);
    db.permissions.set(user.id, consumer.id, ares.newScope);
  ...
  server.grant(oauth2orize.grant.token(function (consumer, user, ares, done) {
    console.log('[grant token]', consumer.id);
    ...
    Auth.BearerLogins.create({ uid: token, info: values }).then(function () {
      done(null, token);
    }, function (err) {
      done(err);
    });
  ...
  server.exchange(oauth2orize.exchange.code(function (consumer, code, redirectURI, done) {
    ...
    if (consumer.appId !== grantValues.appId) { return done(null, false); }
    //if (redirectURI !== authCode.redirectURI) { return done(null, false); }
      
    values = {
      accountId: grantValues.accountId || grantValues.userId
    , userId: grantValues.accountId || grantValues.accountId
    , scope: grantValues.scope
    , appId: grantValues.appId
    };

    // TODO all of these should use realCreate
    Auth.BearerLogin.create({ uid: token, info: values }).then(function () {
      done(null, token);
    }, function (err) {
      done(err);
    });
  ...
  */
  ...
  routes.scopeTokens = [
    function restfulGetScopeDelta(req, res) {
      ...
      getScopeDelta(data.accountId || data.userId, data.appId, data.rawScope).then(function (stuff) {
  ...
  routes.authorization = [
      ...
      function checkAppIdAndUrl(appId, redirectURI, scope, type, done) {
        ...
        Auth.AppLogin.login({ uid: appId }).then(function (appLogin) {
          ...
          */
          ...
          done(null, appLogin, redirectURI);
        }, function (err) {
    ...
    , function checkAuthorizationAndScope(consumer, user, rawScope, done) {
        var account = account && getUserAccount(user)
          ...
          , data = { accountId: accountId, userId: accountId, appId: consumer.id, rawScope: rawScope }
          ;
