...
module.exports.create = function (app, passport, config, DB, Auth) {
  ...
  server.grant(oauth2orize.grant.code(function ($apikey, redirectURI, reqUser, ares, done) {
    ...
    , accounts: accounts
    , requestedScopeArr: authInfo.requestedScopeArr
    ...
    // $apikey.id
    Scopes.set(authInfo.selectedAccountId, $oauthclient.id, authInfo.acceptedScopeString);
  ...
  function route(rest) {
    ...
    , function restfulGetScopeDelta(req, res) {
        ...
        */
        ...
        return Scopes.lookup($account.id, authInfo.oauthclientUuid).then(function (grantedScope) {
          ...
          return getScopeDelta(grantedScope, authInfo.requestedScopeArr)
            .then(function (perms) {
    ...
    function checkAuthorizationAndScope($apikey, user, originalScopeArr, done) {
      ...
      , oauthclientUuid: $oauthclient.id
      , loginId: undefined
      , requestedScopeArr: originalScopeArr
