...
module.exports.create = function (app, passport, config, DB, Auth) {
  ...
  */
  ...
  function getScopeDelta(userId, consumerId, rawScope) {
    return new Promise(function (resolve/*, reject*/) {
      ...
      , consumerId
      , { rawScope: rawScope }
      , function (err, newScope, invalids, reqScope) {
          var info
            ;
  ...
  routes.scopeTokens = [
    function restfulGetScopeDelta(req, res) {
      ...
      getScopeDelta(data.uid || data.userId, data.appId, data.rawScope).then(function (stuff) {
        res.send(stuff);
      }, function (err) {
  ...
  routes.authorization = [
    ...
    , function checkAuthorizationAndScope(consumer, user, rawScope, done) {
        var userId = user && user.id
          , data = { uid: userId, userId: userId, appId: consumer.id, rawScope: rawScope }
          ;
  ...
  //  login.ensureLoggedIn() // the browser app will handle this
    server.decision(function parseDecision(req, done) {
      ...
      // TODO pick apart which scopes were granted and such
